...
| **Model Quantization (4-bit NF4)** | **[지능은 유지하되 무게는 압축]** <br> 16비트(FP16) 정밀도의 모델을 그대로 로드하면 VRAM 부족으로 실행조차 불가능합니다. 이를 **4비트(NF4) 형식으로 압축(Quantization)** 하여 메모리 점유율을 약 1/4로 줄였습니다. 특히 LLM의 가중치 분포에 최적화된 NF4(Normal Float 4: load_in_4bit=True) 방식을 사용하여, 모델의 '똑똑함'은 보존하면서 메모리 효율만 극대화했습니다. |
